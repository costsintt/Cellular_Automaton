...
SDL_Renderer* kiss_init(char* title, kiss_array *a, int w, int h)
{
	...
	{
		printf("Something went wrong in kiss_init...\n");
		SDL_Quit();
		return NULL;
	}
	kiss_screen_width = w;
	...
	r += kiss_font_new(&kiss_textfont, "../fonts/UbuntuMono-Regular.ttf",
	                   a, kiss_textfont_size);
	r += kiss_font_new(&kiss_buttonfont, "../fonts/UbuntuMono-Regular.ttf",
	                   a, kiss_buttonfont_size);
	r += kiss_image_new(&kiss_normal, "../images/kiss_normal.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_prelight, "../images/kiss_prelight.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_active, "../images/kiss_active.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_bar, "../images/kiss_bar.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_vslider, "../images/kiss_vslider.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_hslider, "../images/kiss_hslider.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_up, "../images/kiss_up.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_down, "../images/kiss_down.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_left, "../images/kiss_left.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_right, "../images/kiss_right.png",
	                    a, renderer);
	r += kiss_image_new(&kiss_combo, "../images/kiss_combo.png",
	                    a, renderer);
	...
	if (a->length)
		for (i = a->length - 1; i >= 0; i--)
		{
			if (kiss_array_id(a, i) == FONT_TYPE)
				TTF_CloseFont((TTF_Font *)kiss_array_data(a, i));
			else if (kiss_array_id(a, i) == TEXTURE_TYPE)
				SDL_DestroyTexture((SDL_Texture *)kiss_array_data(a, i));
			else if (kiss_array_id(a, i) == RENDERER_TYPE)
				SDL_DestroyRenderer((SDL_Renderer *)kiss_array_data(a, i));
			else if (kiss_array_id(a, i) == WINDOW_TYPE)
				SDL_DestroyWindow((SDL_Window *)kiss_array_data(a, i));
			else if (kiss_array_id(a, i) == ARRAY_TYPE)
				kiss_array_free((kiss_array *)kiss_array_data(a, i));
			else
				free (a->data[i]);
		}
	a->length = 0;
