...
{
	if(kiss_selectbutton_event(select2, e, draw)) select1->selected ^= 1;
}
	                       ...
	                       kiss_selectbutton *select2, kiss_hscrollbar *hscrollbar, int *draw)
{
	...
	if (kiss_combobox_event(combobox, e, draw))
    {
        ...
        else strcpy(stext, "Data not found");
...
int main(int argc, char **argv)
{
	...
	kiss_label_new(&label1, &window, "Population", kiss_screen_width / 2 -
                  (combobox_width + kiss_up.w - kiss_edge) / 2 + kiss_edge,
		           6 * kiss_textfont.lineheight);
	kiss_selectbutton_new(&select1, &window, label1.rect.x + combobox_width + kiss_up.w -
